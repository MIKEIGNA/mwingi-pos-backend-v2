...
class TpProductPosIndexView(UserActivityLogMixin, generics.ListCreateAPIView, ApiProductViewMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return super(TpProductPosIndexView, self).get(request, *args, **kwargs)

    def get_queryset(self):
        """
        Make sure only the owner can view his/her store
        """
        queryset = super(TpProductPosIndexView, self).get_queryset()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            self.profile = Profile.objects.get(user__email=self.request.user)
            self.store = None
            ...
            try:
                ...
                """ Confirm if modifiers belongs to the store"""
...
class TpProductPosEditView(
    ...
    def get_object(self):
        ...
        self.obj = super(TpProductPosEditView, self).get_object()
        return self.obj
    ...
    def get_queryset(self):
        ...
        Make sure only the owner can view his/her store
        """
        queryset = super(TpProductPosEditView, self).get_queryset()
        queryset = queryset.filter(
            stores__reg_no=self.kwargs['store_reg_no'],
            reg_no=self.kwargs['reg_no'],
            is_variant_child=False,
    ...
    def perform_update(self, serializer): 
        ...
        units = serializer.validated_data['stocks_units']
        minimum_stock = serializer.validated_data['minimum_stock_level']

        StockLevel.perform_item_edit(
...
class TpProductPosImageEditView(
    ...
    def get_object(self):
        ...
        return super(TpProductPosImageEditView, self).get_object()

    def get_queryset(self):
        """
        Make sure only the owner can view his/her store
        """
        queryset = super(TpProductPosImageEditView, self).get_queryset()
        queryset = queryset.filter(
            stores__reg_no=self.kwargs['store_reg_no'],
            reg_no=self.kwargs['reg_no'],
            is_variant_child=False,
