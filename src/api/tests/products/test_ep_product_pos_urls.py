...
class EpProductPosIndexViewTestCase(APITestCase, InitialUserDataMixin):
    ...
    def test_if_view_returns_the_products_correctly(self):
        ...
        with self.assertNumQueries(13):
            response = self.client.get(
                reverse('api:ep_pos_product_index', args=(self.store1.reg_no,)))
        ...
        results1 = json.loads(json.dumps(response.data, cls=DecimalEncoder))['results']
    ...
    def test_if_view_returns_the_products_even_for_employee_without_add_permission(self):
    ...
    def test_if_view_does_not_return_the_current_product_stock_units_from_the_current_store(self):
    ...
    def test_view_when_there_is_a_product_with_variants(self):
        ...
        with self.assertNumQueries(17):
            response = self.client.get(reverse('api:ep_pos_product_index', args=(self.store1.reg_no,)))
            self.assertEqual(response.status_code, 200)
    ...
    def test_if_view_can_handle_wrong_store_reg_no(self):

        response = self.client.get(reverse('api:ep_pos_product_index', args=(4646464,)))
    ...
    def test_if_view_returns_the_right_content_with_pagination(self):
        ...
        self.assertEqual(
            response_data_dict['next'], f'http://testserver/api/pos/ep/products/{self.store1.reg_no}/?page=2')
        ...
        response = self.client.get(reverse('api:ep_pos_product_index', args=(self.store1.reg_no,)) + '?page=2')
        self.assertEqual(response.status_code, 200)

        result = {
            'count': model_num_to_be_created, 
            'next': None, 
            'previous': f'http://testserver/api/pos/ep/products/{self.store1.reg_no}/', 
            'results': [
                {
