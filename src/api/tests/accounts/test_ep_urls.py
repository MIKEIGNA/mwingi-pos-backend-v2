...
class TokenViewForEmployeeUserTestCase(APITestCase, InitialUserDataMixin):
    ...
    def test_if_TokenView_is_working_for_manager_user(self):

        data = {
            'username': 'gucci@gmail.com',
            'password': 'secretpass',
        }
        ...
        self.assertEqual(self.manager_profile1.user.is_active, True)
        ...
        result = {
            'email': self.manager_profile1.user.email,
            'name': self.manager_profile1.user.get_full_name(),
            'token': token.key,
            'user_type': self.manager_profile1.user.user_type,
            'reg_no': self.manager_profile1.user.reg_no,
            'profile_image_url': self.manager_profile1.user.get_profile_image_url(),
    ...
    def test_if_TokenView_is_working_for_employee_user(self):
        ...
        result = {
            'email': self.cashier_profile1.user.email,
            'name': self.cashier_profile1.user.get_full_name(),
            'token': token.key,
            'user_type': self.cashier_profile1.user.user_type,
            'reg_no': self.cashier_profile1.user.reg_no,
            'profile_image_url': self.cashier_profile1.user.get_profile_image_url(),
            'loyalty_value': loyalty_value,
            'general_setting': {
                'enable_shifts': False, 
                'enable_open_tickets': False, 
    ...
    def test_if_TokenView_wont_work_when_user_is_not_active(self):
        ...
        u = User.objects.get(email='gucci@gmail.com')
        u.is_active = False
        u.save()
...
class LogoutViewForEmployeeUserTestCase(APITestCase, InitialUserDataMixin):
    ...
    def setUp(self):
        ...
        * Top User 2 assets:
        ...
        token1 = Token.objects.get(user__email='gucci@gmail.com')
    ...
    def test_if_LogoutView_is_working(self):
        ...
        token2 = Token.objects.get(user__email='gucci@gmail.com')
...
class PasswordChangeViewForEmployeeUserTestCase(APITestCase, InitialUserDataMixin):
    # Test if TokenView urls  #
    def setUp(self):
        """
        This function is defined in the 'InitialUserDataMixin' mixin
    ...
    def test_if_PasswordChangeView_can_change_a_password(self):
        ...
        user = User.objects.get(email=self.manager_profile1.user.email)
...
class ContactViewForEmployeeUserTestCase(APITestCase, InitialUserDataMixin):
